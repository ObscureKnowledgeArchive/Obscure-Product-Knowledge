---
title: "Notches Missing From Pac-Man and Ghost Graphics"
description: "Firsthand Original Pac-Man PCB repair: Notches Missing From Pac-Man and Ghost Graphics. Includes symptoms, diagnostic steps, root cause, PCB locations,..."
layout: default
breadcrumbs:
  - name: Arcade
  - name: Pac-Man & Ms. Pac-Man
  url: /arcade/pac-man-ms-pac-man/
  - name: PCB Repairs
permalink: "/arcade/pac-man-ms-pac-man/repairs/pac-man-notches-missing-sprites-5e-5f/"
category: "Pac-Man / Ms. Pac-Man PCB Repair"
keywords: "Pac-Man notch; ghost head notch; missing sprite pixels; 5E; 5F; masked ROM; character graphics"
---
# Notches Missing From Pac-Man and Ghost Graphics


## Repair Case Summary

This page documents a firsthand **Notches Missing From Pac-Man and Ghost Graphics** repair case.
It is intended for technicians and collectors troubleshooting original Pac-Man or Ms. Pac-Man arcade PCB hardware.

- **Date:** 2017-04-07
- **Board:** Original Pac-Man PCB
- **Category:** Graphics / character ROMs
- **Symptoms:**
  - Pac-Man had a notch missing on the side opposite his mouth.
  - All ghosts had a notch missing from the tops of their heads.
- **Diagnostic observations / tests:**
  - Original masked ROMs were in use.
  - Swapping the suspect ROMs removed the problem.
- **Components / locations investigated:** 5E, 5F
- **Root cause:** Bad 5E/5F masked ROMs.
- **Repair:** Replaced the original masked ROMs at 5E/5F with known-good ROMs.
- **Result:** Pac-Man and ghost graphics returned to normal.
- **Search terms:** Pac-Man notch; ghost head notch; missing sprite pixels; 5E; 5F; masked ROM; character graphics
- **Evidence:** Physically verified repair / firsthand repair log

## Related Pac-Man / Ms. Pac-Man Repair Cases

- [Sparkles or Stars Around Pac-Man and Ghosts](/arcade/pac-man-ms-pac-man/repairs/pac-man-sparkles-stars-5e-5f/)
- [Pinky Jitters During Vertical Movement](/arcade/pac-man-ms-pac-man/repairs/pac-man-pinky-jitter-3h/)
- [Garbled Pac-Man, Ghost and Fruit Sprites](/arcade/pac-man-ms-pac-man/repairs/pac-man-garbled-sprites-74ls174-1h/)
- [Pac-Man and Ghosts Stuck in Upper-Left Corner](/arcade/pac-man-ms-pac-man/repairs/pac-man-characters-upper-left-7489-2f/)

## How to Use This Repair Record

This is a firsthand physical repair case. The same symptom on another Pac-Man or Ms. Pac-Man PCB can have a different electrical cause. Use the documented diagnostic observations and tests to guide troubleshooting rather than replacing a component solely because it repaired this board.

[← Back to the Pac-Man / Ms. Pac-Man PCB Repair Archive](/arcade/pac-man-ms-pac-man/)
